...
float3 GetNoiseOnPosition(float4 vertex, float3 normal) {
	...
	for (int i = 0; i < noiseVolumeCount * 28; i += 28) {
		...
		if (volCoeff < 0) {
			...
			if (noiseVolumeSettings[i + 13] == 1 ) { //clamp to volume
				if (sdGlobal(noiseVolumeSettings[i + 15], float4(mul(unity_ObjectToWorld, vertex).xyz + direction * currentNoiseValue, 1), currentVolumeTransform) > 0) {
					currentNoiseValue = raymarchToEdge(i, direction * sign(currentNoiseValue), mul(unity_ObjectToWorld, vertex).xyz) * sign(currentNoiseValue);
				}
			}
	...
	for (int j = 0; j < noiseVolumeCount * 28; j += 28) {
		if (noiseVolumeSettings[j] == -1) { //mask
			float volCoeff = sdGlobal(noiseVolumeSettings[j + 15], float4(mul(unity_ObjectToWorld, vertex).xyz, 1), noiseVolumeTransforms[(uint)(j / 27)]);
			volCoeff = -volCoeff;
			volCoeff = max(volCoeff, 0);
